...
def test_details_on_home_page(app):
    ...
    assert clean(contact_from_home_page.all_phones) == clean(contact_from_edit_page.all_phones)
    assert clean(contact_from_home_page.all_emails) == clean(contact_from_edit_page.all_emails)
    assert clean(contact_from_home_page.last_name) == clean(contact_from_edit_page.last_name)
    assert clean(contact_from_home_page.first_name) == clean(contact_from_edit_page.first_name)
    assert clean(contact_from_home_page.address) == clean(contact_from_edit_page.address)
...
def test_details_on_home_page(app, db):
    ...
    for contact_from_db, contact_from_home_page in zip(sorted(contacts_from_home_page), sorted(contacts_from_db)):
        assert clean(contact_from_home_page.all_phones) == clean(contact_from_db.all_phones)
        assert clean(contact_from_home_page.all_emails) == clean(contact_from_db.all_emails)
        assert clean(contact_from_home_page.last_name) == clean(contact_from_db.last_name)
        assert clean(contact_from_home_page.first_name) == clean(contact_from_db.first_name)
        assert clean(contact_from_home_page.address) == clean(contact_from_db.address)
